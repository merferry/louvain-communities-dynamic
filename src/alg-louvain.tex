...
\ignore{
\begin{algorithm}[hbtp]
\caption{\textit{Dynamic Frontier Louvain} (\FroLou{}).}
\label{alg:louvain}
\begin{algorithmic}[1]
\Function{\FroLou{}}{$G^t, \Delta^{t-}, \Delta^{t+}, C^{t-1}, K^{t-1}, \Sigma^{t-1}$} \label{alg:louvain--main-begin}
  \State $\rhd$ Initial marking phase
  \State \Fro{}\textsc{-initial}($G^t, \Delta^{t-}, \Delta^{t+}, C^{t-1}$) \label{alg:louvain--mark} \Comment{See Alg. \ref{alg:frontier}}
  ...
  \Return{$C^{t-1} \gets C, K^{t-1} \gets K, \Sigma^{t-1} \gets \Sigma$} \label{alg:louvain--main-return}
\EndFunction \label{alg:louvain--main-end}
