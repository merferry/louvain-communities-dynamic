\begin{figure}[!hbt]
  \centering
  % \subfigure{
  %   \label{fig:strong-scaling--temporal}
  %   \includegraphics[width=0.98\linewidth]{out/strong-scaling-temporal.pdf}
  % }
  \subfigure{
    \label{fig:strong-scaling--8020}
    \includegraphics[width=0.98\linewidth]{out/strong-scaling-8020.pdf}
  } \\[-2ex]
  \caption{Mean speedup of our \textit{Dynamic Frontier (DF)} Louvain with increasing number of threads (in multiples of $2$), on large (static) graphs with random batch updates of size $10^{-7}|E|$ to $0.1|E|$.}
  \label{fig:strong-scaling}
\end{figure}
